...
export function powerToLux(watts: number | undefined | null): number {
  if (watts === undefined || watts === null || !Number.isFinite(watts)) {
    return HOMEKIT_LUX_MIN;
  }
  const abs = Math.abs(watts);
  if (abs < HOMEKIT_LUX_MIN) return HOMEKIT_LUX_MIN;
  if (abs > HOMEKIT_LUX_MAX) return HOMEKIT_LUX_MAX;
  return abs;
}
